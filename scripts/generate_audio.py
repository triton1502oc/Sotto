...
def build_soundtrack(total_duration_sec, events, output_wav):
    total_samples = int(total_duration_sec * SAMPLE_RATE)
    timeline = [0] * total_samples

    for timestamp_sec, wav_path in events:
    ...
    write_wav(output_wav, timeline)
    print(f"Generated soundtrack {output_wav} ({total_duration_sec}s)")
...
if __name__ == '__main__':
    ...
    tts_to_wav("I need a moment, please.", 'demo/audio_scratch/moment.wav')
    tts_to_wav("I cannot speak right now. Please read my screen.", 'demo/audio_scratch/emergency.wav')
    tts_to_wav("Hello, this is my speaking voice.", 'demo/audio_scratch/test_voice.wav')

    # Combine chime + silence + test_voice
    chime_test = chime_samples + [0] * int(0.28 * SAMPLE_RATE) + read_wav('demo/audio_scratch/test_voice.wav')
    write_wav('demo/audio_scratch/chime_test_voice.wav', chime_test)
